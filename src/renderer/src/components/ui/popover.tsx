...
interface PopoverContextType {
  ...
  dragStarted: React.RefObject<boolean> // <-- Add this
}
...
function usePopoverLogic(dragStarted: React.RefObject<boolean>) {
  ...
  useEffect(() => {
    window.electron.ipcRenderer.on('close-popover', () => {
      closePopover()
    })
    return () => {
      window.electron.ipcRenderer.removeAllListeners('close-popover')
    }
  }, [])

  return { isOpen, openPopover, closePopover, uniqueId, note, setNote, dragStarted }
}

interface PopoverRootProps extends React.ComponentProps<typeof motion.div> {
  children: React.ReactNode
  className?: string
  dragConstraintsMainRef: React.RefObject<HTMLDivElement | null>
}

export function PopoverRoot({
  children,
  className,
  dragConstraintsMainRef,
  ...props
}: PopoverRootProps) {
  const dragStarted = useRef(false)
  const controls = useDragControls()

  const popoverLogic = usePopoverLogic(dragStarted)
    ...
    <PopoverContext.Provider value={popoverLogic}>
      <MotionConfig transition={TRANSITION}>
        <motion.div
          ...
          dragMomentum={false}
          dragElastic={0}
          dragConstraints={dragConstraintsMainRef}
          onDragStart={() => {
...
interface PopoverTriggerProps extends React.ComponentProps<typeof motion.button> {
  children: React.ReactNode
  className?: string
}

export function PopoverTrigger({ children, className, ...props }: PopoverTriggerProps) {
  const { openPopover, uniqueId, dragStarted } = usePopover()
...
interface PopoverContentProps extends React.ComponentProps<typeof motion.div> {
  children: React.ReactNode
  className?: string
}
